...
| ------------------------------ | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Ana Luiza do Santos - 5980** | Lógica de negócio central         | Modelagem de dados (`models.py`: Corrida, Motorista, Passageiro); máquina de estados da corrida (`request → match → confirm → in_transit → complete`) em `routes/rides.py`; CRUD e atribuição de motoristas (`routes/drivers.py`); autenticação e gestão de passageiros (`routes/auth.py`); testes unitários (`test_corridas.py`, `test_motoristas.py`, `test_auth.py`)                                                                           |
| **Meishan Huang - 8761**       | Infraestrutura distribuída        | Logging estruturado em JSON (`logger.py`); health check e política de overflow/congestionamento (`fila.py`); fila de corridas via RabbitMQ (entrada/saída); painel administrativo (`routes/admin.py` + tela `Admin.jsx`)                                                                                                                                                                                                                          |
