...
| **Meishan Huang - 8761**   | Infraestrutura distribuída        | Logging estruturado em JSON (`logger.py`); health check e política de overflow/congestionamento (`fila.py`); fila de corridas via RabbitMQ (entrada/saída); load balancer nginx com 2 instâncias de backend (`nginx.conf`); painel administrativo (`routes/admin.py` + tela `Admin.jsx`)                                                                                           |
| **Gabriel Menezes - 8763** | Integração com o Core             | Cliente HTTP do contrato (`core_client.py`): registro do grupo, leilão, locks distribuídos, avanço de saga, relógio de Lamport; webhooks de leilão (`/rides/incoming`, `/rides/{uuid}/assigned`); lógica de delegação de saída/entrada e vigia de corridas delegadas; testes de contrato (`test_contrato.py`, `test_vigia.py`); compensação de saga e resiliência a falhas do Core |
